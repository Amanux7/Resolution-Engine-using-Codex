...
async function readState():Promise<State>{try{const state=JSON.parse(await readFile(statePath,"utf8")) as Partial<State>;return {...blankState(),...state,recommendations:state.recommendations??[],actionPackages:state.actionPackages??[]};}catch(error){if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error;await mkdir(path.dirname(statePath),{recursive:true});const state=blankState();await writeFile(statePath,JSON.stringify(state,null,2));return state;}}
