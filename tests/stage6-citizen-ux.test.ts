...
describe("Stage 6 citizen journey safeguards",()=>{
  ...
  it("creates a clearly labelled fictional sample without replacing a user-created case",async()=>{const repositories=getRepositories();const personal=await repositories.cases.create(createCaseInput("My service problem needs organizing.","user-demo"));const response=await createSampleCase(new Request("http://localhost/api/demo-case",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sampleId:"consumer-refund"})}));expect(response.status).toBe(200);const body=await response.json() as {case:{id:string;title:string}};expect(body.case.id).not.toBe(personal.id);expect(body.case.title).toContain("Sample case");const original=await repositories.cases.getById(personal.id,"user-demo");expect(original?.description).toContain("service problem");});
